...
export class ClaudeAgentAdapter extends AbstractAgent {
  ...
  private interruptedThreads = new Set<string>();
  private activeSessionUses = new Map<string, number>();
  ...
  protected beginSessionUse(threadId: string): void {
    this.activeSessionUses.set(
      threadId,
      (this.activeSessionUses.get(threadId) ?? 0) + 1,
    );
  ...
  protected endSessionUse(threadId: string): void {
    const activeUses = this.activeSessionUses.get(threadId) ?? 0;
    if (activeUses > 1) {
      this.activeSessionUses.set(threadId, activeUses - 1);
    } else {
      this.activeSessionUses.delete(threadId);
    }

    const idleEntry = this.sessions.get(threadId);
    if (idleEntry && !this.isSessionInUse(threadId)) {
  ...
  protected rememberSession(threadId: string, sessionId: string): void {
    const existing = this.sessions.get(threadId);
    this.sessions.set(threadId, {
      sessionId,
      lastUsed: Date.now(),
      active: this.isSessionInUse(threadId) || existing?.active === true,
    });
    this.evictSessions();
  }

  private isSessionInUse(threadId: string): boolean {
    return (this.activeSessionUses.get(threadId) ?? 0) > 0;
  }
  ...
  public async interrupt(threadId?: string): Promise<void> {
    if (threadId) {
      const queryStream = this.activeQueries.get(threadId);
      if (queryStream) this.interruptedThreads.add(threadId);
      await queryStream?.interrupt();
      return;
    }
    const activeQueries = [...this.activeQueries.entries()];
    for (const [activeThreadId] of activeQueries) {
      this.interruptedThreads.add(activeThreadId);
    }
    await Promise.allSettled(activeQueries.map(([, q]) => q.interrupt()));
  }
  ...
  run(input: RunAgentInput): Observable<BaseEvent> {
    return new Observable<ProcessedEvent>((subscriber) => {
      ...
      this.translateStream(runInput, queryStream, subscriber)
        ...
        .finally(() => {
          if (timeoutHandle !== undefined) clearTimeout(timeoutHandle);
          if (this.activeQueries.get(threadId) === queryStream) {
  ...
  private async translateStream(
    ...
    try {
      ...
      if (this.interruptedThreads.has(threadId)) {
        subscriber.complete();
        return;
      }
  ...
  private async streamMessages(
    ...
    // Track all active tool calls (not just the current one) so we can close
    // them in the finally block. The Claude Agent SDK's agentic loop may emit
    // multiple TOOL_CALL_START events in one turn (parallel tool use) but only
    // deliver content_block_stop for the last one, leaving the rest dangling.
    ...
    try {
          ...
          if (eventType === "message_start") {
            ...
            if (deltaType === "text_delta") {
              ...
              if (partialJson && currentToolCallId) {
                ...
                // Keep the active map entry's json in sync
                const entry = activeToolCallIds.get(currentToolCallId);
                if (entry) entry.json = accumulatedToolJson;
                subscriber.next({
            ...
            if (block.type === "thinking") {
              ...
              if (currentToolCallId) {
                ...
                // Track in active map so we can close it later if content_block_stop never arrives
            ...
            if (currentToolCallId) {
              // Remove from active map — this tool call is now properly closed
        ...
        // Handle system messages
          ...
          if (subtype === "init") {
            const sid = (raw.session_id ?? data?.session_id) as
              ...
              | undefined;
            if (sid) {
              this.rememberSession(threadId, sid);
              console.debug(
      ...
      // Close ALL active tool calls that never received content_block_stop.
      // The Claude Agent SDK's agentic loop may emit multiple TOOL_CALL_START
      // events in one turn (parallel tool use) but only deliver
      // content_block_stop for the last one, leaving the rest dangling.
      for (const [toolCallId] of activeToolCallIds) {
        console.debug(
          `[ClaudeAdapter] Cleanup: closing hanging TOOL_CALL_START for ${toolCallId}`,
        );
        subscriber.next({
          type: EventType.TOOL_CALL_END,
          threadId,
