...
- CI 扩展：`pnpm --filter @agework/adapters typecheck|test`、`@agework/runtime typecheck|test`、`server test`、`web test`、`pnpm build`、**Codex Schema drift check**（§9）。真实模型 smoke 独立触发/nightly，不卡普通 PR。
