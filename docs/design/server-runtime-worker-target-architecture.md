...
>
> **⚠️ 更新（2026-07-19）**：本文档中关于 `OwnerKey`/`WorkerKey`、`releaseOwner`、`listOwners` 等 owner 边界设计已被 [`runtime-owner-boundary.md`](runtime-owner-boundary.md) 全面取代。该定案移除了公共协议中的 `OwnerKey`/`WorkerKey`，改由 Server 下发业务事实（`scope + userId + workspaceId + userLifecycleVersion`），Runtime 独立派生 `ReuseIdentity` 并决定复用策略。本文档的以下部分与该定案冲突，以后者为准：
...
>
> 本文仍是三层关系（Server → Runtime Host → Worker）和执行面隔离原则的权威参考；owner 边界细节请参阅 [`runtime-owner-boundary.md`](runtime-owner-boundary.md)。
