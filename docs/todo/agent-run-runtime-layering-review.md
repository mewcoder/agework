...
> 8→5 依赖。其余 Step（C/D/E 的目录与门面）在更早的模块单数化重构里已完成。
>
> **Update (2026-06-28)**：run→runtime 内部依赖收口（原 P2-7 KNOWN_BOUNDARY_DEBT）已完成——
> sandbox 执行编排迁入 `runtime/sandbox/sandbox-worker.executor.ts`，由 `RuntimeService` 的
> `startSandboxWorker / cancelSandboxRun / cleanupSandboxRun / setSandboxWorkerEventSink` 等门面暴露；
> run 的 `SandboxRunExecutor` 改为薄适配器，`RunStartupService` 经 `RuntimeService.setSandboxOwnerSessionCleanup`
> 接线。`common/module-boundary.spec.ts` 的 KNOWN_BOUNDARY_DEBT allowlist 已整体删除。剩 §6 import 规则 CI（Step F）未做。
