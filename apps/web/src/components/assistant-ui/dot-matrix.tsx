...
// Keep the animation deterministic so server and client render the same markup.
...
const STATES = {
  ...
  searching: {
    blink: (_i: number, _row: number, col: number) => ({ duration: 1.1, delay: -col * 0.12, lo: 0.2 }),
  },
  syncing: {
    blink: (_i: number, row: number, col: number) => {
      const turn =
  ...
  downloading: {
    blink: (_i: number, row: number) => ({ duration: 1, delay: -row * 0.12, lo: 0.2 }),
  },
  listening: {
    blink: (_i: number, _row: number, col: number) => ({
