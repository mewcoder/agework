 ...
 * 场景:新启动的 owner 还没来得及第一次 poll 就被 watchdog 扫到、以及 server
...
export class WorkerLivenessStore {
  ...
  touch(ownerId: string): void {
    this.seenAt.set(ownerId, Date.now());
  }

  lastSeenAt(ownerId: string): number | undefined {
    return this.seenAt.get(ownerId);
  }

  remove(ownerId: string): void {
    this.seenAt.delete(ownerId);
  }

  listStale(thresholdMs: number, now: number): string[] {
    const cutoff = now - thresholdMs;
    const stale: string[] = [];
    for (const [ownerId, seenAt] of this.seenAt) {
      if (seenAt < cutoff) stale.push(ownerId);
    }
    return stale;
  }
}
