...
describe("WorkspaceHostListener", () => {
  it("releases only the deleted workspace owner", async () => {
    const hostContract = {
    ...
    await listener.onWorkspaceDeleted(
      new WorkspaceDeletedEvent("ws-1", "user-1")
    );

    expect(hostContract.releaseOwner).toHaveBeenCalledTimes(1);
    expect(hostContract.releaseOwner).toHaveBeenCalledWith("workspace:ws-1");
    expect(hostContract.releaseOwner).not.toHaveBeenCalledWith("user:user-1");
  });
});
