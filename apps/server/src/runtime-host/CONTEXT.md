...
server 管理 RuntimeHost 的注册、存储与调度入口（节点资源域:注册行、配对、隧道、判死、envConfig、目录浏览）。RuntimeHost 是 worker 运行的载体——builtin（本机 in-process）或 registered（远程机器注册）。下发面在 `host-dispatch/` 模块。
