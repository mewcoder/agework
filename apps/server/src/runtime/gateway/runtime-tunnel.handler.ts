 ...
 * 配对 token(sha256 比对)鉴权,处理 register/heartbeat,断连即标 offline。
 * server 永不反连 runtime——这里只被动收连接。Phase 2 在此连接上叠加 launch/stop RPC。
 */
@Injectable()
  ...
  implements OnApplicationBootstrap, OnApplicationShutdown
{
  ...
  private async onMessage(
    ...
    let message: RuntimeTunnelClientMessage;
    try {
      message = JSON.parse(text) as RuntimeTunnelClientMessage;
    } catch {
