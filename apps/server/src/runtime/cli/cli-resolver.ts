/**
 * CLI 环境检测（server 端本地检测副本）。
 * 逻辑与 apps/runtime/src/cli/cli-resolver.ts 保持一致，
 * 用于 builtin runtime 的本地 envConfig 检测。
 */

import { spawnSync, execSync } from "node:child_process";
import type { AgentDetectedEnv, RuntimeEnvConfig } from "@agework/shared/api";
import { claudeKnownLocations, codexKnownLocations } from "./cli-known-paths";

/**
 * 检测本机 agent CLI 环境，返回完整的 envConfig（claude + codex）。
 * builtin runtime 启动时和 admin 触发重检时各调一次。
 */
